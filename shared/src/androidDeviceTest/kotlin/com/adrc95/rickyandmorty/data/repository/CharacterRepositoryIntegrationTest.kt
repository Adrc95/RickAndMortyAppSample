...
class CharacterRepositoryIntegrationTest : KoinTest {
    ...
    fun givenRemoteCharacter_whenGetCharacterDetail_thenReturnsCharacterAndUsesExpectedPath() = runTest {
        mockWebServerRule.server.enqueue(
            MockResponse()
                .setResponseCode(200)
                .setHeader("Content-Type", "application/json").setBody("character_detail.json".readJsonAsset())
        )
    ...
    fun givenRemoteFailureAndNoCache_whenGetCharacterDetail_thenReturnsServerError() = runTest {
        mockWebServerRule.server.enqueue(MockResponse().setResponseCode(500).setHeader("Content-Type", "application/json").setBody("{}"))
    ...
    fun givenRemoteCharacters_whenGetCharacters_thenPersistsAndEmitsCharacters() = runTest {
        mockWebServerRule.server.enqueue(
            MockResponse()
                .setResponseCode(200)
                .setHeader("Content-Type", "application/json").setBody("characters_page_1.json".readJsonAsset())
        )
        val differ = TestPagingDataDiffer<Character>()
        ...
        differ.onPagesUpdatedFlow.first {
            mockWebServerRule.server.requestCount == 1 && differ.snapshot().isNotEmpty()
        }
    ...
    fun givenSearchFilters_whenSearchCharacters_thenSendsFiltersAndPersistsResult() = runTest {
        mockWebServerRule.server.enqueue(
            MockResponse()
                .setResponseCode(200)
                .setHeader("Content-Type", "application/json").setBody("search_characters.json".readJsonAsset())
        )
        val differ = TestPagingDataDiffer<Character>()

        repository.searchCharacters("rick", "Human", "Male", "Alive").test {
            val submitJob = launch { differ.collectFrom(awaitItem()) }
            differ.onPagesUpdatedFlow.first()
            submitJob.cancel()
    ...
    fun givenSearchReturnsNotFound_whenSearchCharacters_thenEmitsEmptyPage() = runTest {
        mockWebServerRule.server.enqueue(MockResponse().setResponseCode(404).setHeader("Content-Type", "application/json").setBody("{}"))
        val differ = TestPagingDataDiffer<Character>()
    ...
    fun givenLastRemotePage_whenGetCharacters_thenDoesNotRequestThirdPage() = runTest {
        mockWebServerRule.server.enqueue(
            MockResponse().setResponseCode(200).setHeader("Content-Type", "application/json").setBody("characters_page_1.json".readJsonAsset())
        )
        val differ = TestPagingDataDiffer<Character>()
    ...
    fun givenSearchQueryWithSpecialCharacters_whenSearchCharacters_thenEncodesRequestQuery() = runTest {
        ...
        assertEquals(
            "/character?page=1&name=rick+%26+morty",
            mockWebServerRule.server.takeRequest().path
        )
    }
