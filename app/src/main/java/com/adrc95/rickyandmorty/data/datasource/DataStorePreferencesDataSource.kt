...
class DataStorePreferencesDataSource @Inject constructor(
    ...
    inline fun <reified T> getFlow(
        key: String,
        default: T
    ): Flow<T> = dataStore.data.map { prefs -> prefs.getOrDefault<T>(key, default) }

    suspend inline fun <reified T> getOnce(
        key: String,
        default: T
    ): T = dataStore.data.map { prefs -> prefs.getOrDefault<T>(key, default) }.first()
